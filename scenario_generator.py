...
def scenario_generator(q, m, n, noj, layout_filepath, de, wh, rhy, pla):
    ...
    order = [[] for i in range(int(2*workingtime/rhythm))]  # return request
    remainning_order = 0
    ...
    for iter in range(int(2*workingtime/rhythm)):

        if iter < (workingtime/(2*rhythm)) :
            new_demand = demand_generator(m, n, noj, rhythm, total_demand_rate, typ = 1)
            num_of_new_order = 0
        ...
        if iter >= ((workingtime/(2*rhythm))-1) and int(sum(sum(demand)))==0 and int(remainning_order) == 0:
            print("demand satisfied at interval %d" %iter)
            print("average waiting time %f" %(total_waiting_time/total_car_num))
            temp = 0
            ...
            while True:
                if os.path.exists(filename):
                    ...
                    break    
            writing = {'res':res, 'Ts':Ts.tolist(), 'rhythm':rhythm}
            with io.open(filename,'w',encoding='utf-8') as json_file:
                json_file.write(unicode(json.dumps(writing, ensure_ascii=False)))
            q.put(res)

            pub.sendMessage("start_simulating", filepath = filename)
            #pub.sendMessage("get_test_data", data = total_waiting_time/total_car_num)
...
##### test
def scenario_generator_new(q, m, n, noj, layout_filepath, de, wh, rhy, pla):
    total_demand_rate = de
    workingtime = wh*60
    rhythm = rhy
    capatity = pla
    ...
    max_inter = int(np.ceil(longest_path/rhythm))*2
    demand = np.zeros((m+n+noj,m+n+noj))
    order = [[] for i in range(int(2*workingtime/rhythm))]  # return request
    remainning_order = 0
    ...
    for iter in range(int(2*workingtime/rhythm)):
        ...
        if iter < (workingtime/(2*rhythm)) :
            new_demand = demand_generator(m, n, noj, rhythm, total_demand_rate, typ=2)
            num_of_new_order = 0
            for item in new_demand:
                demand[item[0]][item[1]] += item[2]
                num_of_new_order += item[2]
        ...
        for i in range(m+n+noj):
            for j in range(m+n+noj):
                if demand[i][j] != 0:
                    ...
                    distri_time = 4
                    offset = dis_matrix[i_temp][j_temp] + distri_time
                    pre_node = i_temp
                    while pre_node != j_temp:
                        temp = int(pre_matrix[j_temp][pre_node])
                        e = int(np.floor((offset + dis_matrix[i_temp][temp]+Ts[i])/rhythm))
                        key = '%d_%d_%d'%(e,temp,pre_node)
                        if key in use_path.keys():
                            use_path[key] = use_path[key] + [(i,j)]
                        else:
